    ...
    riddle.choices.forEach(choice => {
        ...
        li.onclick = () => {
            if (answeringAllowed) {
                ...
                answeringAllowed = false; // Prevent further choices after an answer is selected
            }
        };
        choicesList.appendChild(li);
    });
    answeringAllowed = true; // Allow answering when the riddle is displayed
}
...
function checkAnswer(choice, correctAnswer, selectedLi) {
    ...
    if (choice === correctAnswer) {
        let points = Math.max(100, Math.floor(1000 / timeTaken)); // Calculate points based on time taken
        score += points;
        selectedLi.style.backgroundColor = '#32c292'; // Correct answer
    } else {
        let pointsDeducted = Math.max(50, Math.floor(1000 / timeTaken)); // Calculate points to deduct based on time taken
    ...
    setTimeout(showNextRiddle, 2000); // Wait a second before showing next riddle
}
